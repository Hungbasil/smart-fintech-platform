      ...
      <aside className={`hidden shrink-0 border-r border-[#e3ebe8] bg-[#fbfdfc] py-6 transition-[width,padding] duration-200 lg:flex lg:flex-col ${sidebarCollapsed ? 'sidebar-collapsed w-[76px] px-3' : 'w-[248px] px-5'}`}>
        ...
        <nav className="flex flex-col gap-1">
          ...
          {navigation.map(({ label, to, icon: Icon }) => (
            <NavLink key={to} data-tour={to === '/transactions' ? 'transactions-link' : to === '/wallets' ? 'wallets-link' : to === '/categories' ? 'categories-link' : to === '/budgets' ? 'budgets-link' : to === '/saving-goals' ? 'saving-goals-link' : undefined} to={to} end={to === '/'} className={({ isActive }) => `group flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold no-underline transition-colors ${isActive ? 'bg-[#e4f4f0] text-[#075c57]' : 'text-[#71808c] hover:bg-[#f1f6f4] hover:text-[#17212b]'}`}>
      ...
      <div className="app-main flex-1">
        ...
        <div data-tour="navigation" className="fixed inset-x-0 bottom-0 z-20 lg:hidden">
          ...
          <nav className="flex overflow-x-auto border-t border-[#e3ebe8] bg-[#fbfdfc]/95 px-2 py-2 backdrop-blur [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            <button data-tour="analytics-link" type="button" onClick={() => { setAnalyticsOpen((open) => !open); setOverviewOpen(false); setDebtsOpen(false); }} className={`flex w-[72px] shrink-0 flex-col items-center gap-1 rounded-lg px-1 py-1 text-[10px] font-bold ${location.pathname.startsWith('/analytics') ? 'text-[#087f74]' : 'text-[#9aa7af]'}`}><BarChart3 size={18} />Analytics</button>
            <button data-tour="debts-link" type="button" onClick={() => { setDebtsOpen((open) => !open); setOverviewOpen(false); setAnalyticsOpen(false); }} className={`flex w-[72px] shrink-0 flex-col items-center gap-1 rounded-lg px-1 py-1 text-[10px] font-bold ${location.pathname.startsWith('/debts') ? 'text-[#087f74]' : 'text-[#9aa7af]'}`}><HandCoins size={18} />Debts</button>
            {navigation.map(({ label, to, icon: Icon }) => (
              <NavLink key={to} data-tour={to === '/transactions' ? 'transactions-link' : to === '/wallets' ? 'wallets-link' : to === '/categories' ? 'categories-link' : to === '/budgets' ? 'budgets-link' : to === '/saving-goals' ? 'saving-goals-link' : undefined} to={to} end={to === '/'} className={({ isActive }) => `flex w-[72px] shrink-0 flex-col items-center gap-1 rounded-lg px-1 py-1 text-[10px] font-bold no-underline ${isActive ? 'text-[#087f74]' : 'text-[#9aa7af]'}`}><Icon size={18} />{label}</NavLink>
            ))}
            {isAdmin && (
        ...
        <OnboardingTour forceStart={tourRequest} onForceStartHandled={() => setTourRequest(0)} />
      </div>
    </div>
  );
};
