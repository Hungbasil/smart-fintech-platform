...
import { BarChart3, CalendarClock, ChevronDown, ChevronRight, Goal, HandCoins, LayoutDashboard, LogOut, ReceiptText, Tag, Target, WalletCards } from 'lucide-react';
    ...
    <div className="app-shell flex">
      ...
      <div className="app-main flex-1">
        ...
        <nav className="fixed inset-x-0 bottom-0 z-20 flex justify-around border-t border-[#e3ebe8] bg-[#fbfdfc]/95 px-2 py-2 backdrop-blur lg:hidden">
          <NavLink to="/analytics/overview" className={({ isActive }) => `flex min-w-[68px] flex-col items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-bold no-underline ${isActive ? 'text-[#087f74]' : 'text-[#9aa7af]'}`}><BarChart3 size={18} />Analytics</NavLink>
          {navigation.map(({ label, to, icon: Icon }) => (
