...
import { CalendarClock, FastForward, Pencil, Plus, Trash2, X } from 'lucide-react';
...
interface Recurring { id: string; walletId: string; categoryId: string; description: string; amount: number; dayOfMonth: number; active: boolean; lastProcessed?: string; nextRun: string; skippedMonth?: string; }
...
export const Recurring: React.FC = () => {
  ...
  const skipNext = async (item: Recurring) => {
    if (!window.confirm(`Skip the next cycle for ${item.description}?`)) return;
    try {
      await api.post(`/recurring-transactions/${item.id}/skip-next`);
      await load();
      toast.success('Next recurring cycle skipped');
    } catch (err) {
      toast.error(getApiErrorMessage(err, 'Unable to skip recurring cycle'));
    }
  };
  ...
  return <div>
    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[330px_1fr]">
      ...
      <div className="space-y-4">{items.map((item) => <Card key={item.id}><CardHeader><div className="flex items-start justify-between"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#e4f4f0] text-[#087f74]"><CalendarClock size={18} /></span><div><h2 className="section-title">{item.description}</h2><p className="section-caption mt-1">Day {item.dayOfMonth} each month</p></div></div><div className="flex gap-1"><button aria-label={`Skip next ${item.description}`} title="Skip next cycle" onClick={() => void skipNext(item)} disabled={!item.active} className="rounded-lg p-2 text-[#9aa7af] hover:bg-[#fff4df] hover:text-[#bd7a22] disabled:cursor-not-allowed disabled:opacity-40"><FastForward size={16} /></button><button aria-label={`Edit ${item.description}`} title="Edit schedule" onClick={() => edit(item)} className="rounded-lg p-2 text-[#9aa7af] hover:bg-[#e4f4f0] hover:text-[#087f74]"><Pencil size={16} /></button><button aria-label={`Delete ${item.description}`} title="Delete schedule" onClick={() => remove(item.id)} className="rounded-lg p-2 text-[#9aa7af] hover:bg-[#fff1ef] hover:text-[#d76756]"><Trash2 size={16} /></button></div></div></CardHeader><CardBody><p className="text-lg font-extrabold text-[#17212b]">{currency.format(item.amount)}</p><p className="mt-1 text-xs text-[#71808c]">{wallets.find((wallet) => wallet.id === item.walletId)?.name || 'Wallet'} · {categories.find((category) => category.id === item.categoryId)?.name || 'Category'}</p><p className="mt-3 text-xs font-semibold text-[#087f74]">Next run: {new Date(`${item.nextRun}T00:00:00`).toLocaleDateString('vi-VN')}{item.skippedMonth ? ` · Skipped ${item.skippedMonth}` : ''}</p></CardBody></Card>)}{items.length === 0 && <div className="surface flex min-h-[220px] items-center justify-center p-8 text-center"><div><CalendarClock size={25} className="mx-auto text-[#9aa7af]" /><p className="mt-3 font-bold text-[#17212b]">No schedules yet</p></div></div>}</div>
    </div>
  </div>;
};
